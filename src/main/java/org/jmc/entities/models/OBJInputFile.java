...
public class OBJInputFile
{
	 ...
	 * @throws IOException 
	 */
	public void loadFile(String filename, InputStream objfile, String overwriteName) throws IOException
	{
		objects=new HashMap<>();
		default_object=null;
		...
		while((line=in.readLine())!=null)
		{
			...
			if(line.startsWith("v "))
			{
				...
				try {
					...
					Log.info("ERROR vertex format exception in file "+filename+"["+line_count+"]: "+e);
				}
				finally {
					scanner.close();
				}
			...
			if(line.startsWith("vt "))
			{
				try {
					...
					Log.info("ERROR texture format exception in file "+filename+"["+line_count+"]: "+e);
				}
				continue;
			}
			...
			if(line.startsWith("vn "))
			{
				try {
					...
					Log.info("ERROR normal format exception in file "+filename+"["+line_count+"]: "+e);
				}
				continue;
			}
			...
			if(line.startsWith("f "))
			{
				...
				if(vs.length < 3)
				{
					Log.error("ERROR wrong number of vertices in face in file "+filename+"["+line_count+"]",null);
					continue;
				}
				...
				for(int i=0; i<vs.length; i++)
				{
					try {
						...
						else if(vt.length==3)
						{
							...
							Log.info("ERROR unknown vertex format in file "+filename+"["+line_count+"]");
					} catch (Exception e) {
						Log.info("ERROR unknown vertex format in file "+filename+"["+line_count+"]: "+e);
					}
				}
				if(!has_uv) f.uv=null;
			...
			Log.info("ERROR unknown line in OBJ file "+filename+"["+line_count+"]: "+line);
		}
		in.close();
	}
	
	public void loadFile(String objName, InputStream objfile) throws IOException
	{
		loadFile(objName, objfile, null);
		return;
	}
	...
	public void addObjectToOutput(OBJGroup group, Transform trans, ChunkProcessor out)
	{
		for(OBJFace f:group.faces)
		{
			...
			out.addFace(v, norm, uv, trans, f.mtl, true);
		}
	}
}
