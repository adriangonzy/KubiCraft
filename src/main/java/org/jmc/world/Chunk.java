...
public class Chunk {
	...
	{		
		...
		if(is_anvil)
		{
			...
			for(NBT_Tag section: sections.elements)
			{
				...
				for(int i=0; i<tagBlocks.length(); i++)
					ret.id[base+i] = (short)(tagBlocks.get(i)&0xff);	// convert signed to unsigned

				if(tagAdd!=null)
				{
					for(int i=0; i<tagAdd.length(); i++)
					{
						short add = (short)(tagAdd.get(i)&0xff);	// convert signed to unsigned
				...
				{
					byte add1=(byte)(tagData.get(i)&0x0f);
					byte add2=(byte)(tagData.get(i)>>4);
				...
				if(tagBiomes!=null)
				{
					for(int i=0; i<tagBiomes.length(); i++)
						ret.biome[i]=tagBiomes.get(i);
				}
				else
				{
		...
		else
		{
			...
			byte add1,add2;
			ret=new Blocks(blocks.length(),256);

			for(int i=0; i<blocks.length(); i++)
				ret.id[i]=blocks.get(i);

			for(int i=0; i<data.length(); i++)
			{
				add1=(byte) (data.get(i)&0x0f);
				add2=(byte) (data.get(i)>>4);
				ret.data[2*i]=add1;
				ret.data[2*i+1]=add2;
			}
